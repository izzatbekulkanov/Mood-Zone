$(document).ready(function () {
    $('#save_student_data_base').click(function () {
        createFollowerFromApi();
    });
});

function createFollowerFromApi() {
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.success) {
                    ...
                    save_button_div.innerHTML = `<button class="btn btn-primary w-100 mt-3 p-3 " id="save_student_data_base" onclick=createFollowerFromApi();
                                            type="button"> Saqlash
                                    </button>`
...
$(document).ready(function () {
    ...
    if (studentIdNumber.length > 5) {
      $('#save_button_div').append('<button class="btn btn-success w-100 mt-3 p-3 " id="save_student_data_base" onclick=createFollowerFromApi(); type="button">Saqlash</button>');
}

function fetchStudentInfo(studentIdNumber) {
    var url_id = '/user/get_student_info?student_id_number=';
    $.ajax({
        ...
        success: function (data, status, xhr) {
            if (xhr.status === 200) {
                ...
                save_button_div.innerHTML = `<button class="btn btn-success w-100 mt-3 p-3 " id="save_student_data_base" onclick=createFollowerFromApi();
...
function displayFollowerInfo(data) {
